...
type LOGIN_SELECTION uint8

const (
	SELECTED_LOGIN LOGIN_SELECTION = iota
	SELECTED_PASSWORD
	SELECTED_GO
)
...
type loginData struct {
	login    []rune
	password []rune
	selected LOGIN_SELECTION
}

func connectServer() {

	changeGameMode(MODE_Connect)
		...
		case CMD_WorldUpdate:
			...
			if numObj > 0 {
				...
				for x = 0; x < numObj; x++ {
					...
					var spid uint8
					err = binary.Read(inbuf, binary.LittleEndian, &spid)
					if err != nil {
						doLog(true, "%v", err.Error())
						break
					}
					...
					wObjList = append(wObjList,
						&worldObject{
							itemId:   IID{section: sid, num: oid, sprite: spid},
							pos:      XY{X: nx, Y: ny},
							itemData: objData})
				}
			}
